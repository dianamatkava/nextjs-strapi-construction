...
const InputLineField = ({label, placeholder, name, type="input", required=true, inputClassName="", labelClassName="", children=null}: {label?: string, placeholder: string, required?: boolean, children?: React.ReactElement}) => {
  return (
    <div className={`border border-black rounded-3xl relative w-full flex flex-col items-start justify-start text-xs gap-2`}>
      {label &&
        <label
      ...
      <input name={name} type={type} className={`border border-black rounded-3xl pl-10 py-3 text-sm px-2 w-full ${inputClassName}`}
